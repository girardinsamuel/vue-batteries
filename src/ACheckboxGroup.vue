      ...
      <div class="max-w-lg space-y-4">
        <div class="relative flex items-start" v-for="option in normalizedOptions" :key="option.value">
          <div class="flex items-center h-5">
            <input @change="toggleOption(option)" type="checkbox" :checked="isSelected(option)" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500 rounded">
          </div>
...
export default {
  ...
  setup (props, context) {
    ...
    return {
      ...useSelect(context, modelValue, options, true),
      hasError,
      errorMessage,
    }
  },
